...
import { organization } from "../../../../../utils/iconmap";
export const Organization=()=>{
  const navigaror=useNavigate()
    const menus = data.switchMenu.find((item) => item.id === "outlets").menus;
  const submenus = menus.find((item) => item.id === "menu-fifth").subMenus.find((item)=>item.id==="submenu-two").subsubmenu;
  return(
    <>
     <div className="my-10 mx-5 grid grid-cols-1 gap-6 justify-center lg:grid-cols-4 md:grid-cols-3">
                    {submenus?.filter((menu) => !menu.hidden).length > 0 ? (
                      submenus
                        ...
                        .map((menu) => {
                          const Icon =organization[menu.icon]
