...
export const Subsubmenu = () => {
  const first_stage_menu =
    data.switchMenu
      ...
      .menus?.find((sid) => sid.id === "record-fifith")
      .subMenus?.find((submid) => submid.id === "item-one").subsubmenus || [];
...
export const SecondSubsubmenu=()=>{
    const first_stage_menu =
    data.switchMenu
      .find((mid) => mid.id === "finance")
      .menus?.find((sid) => sid.id === "record-fifith")
      .subMenus?.find((submid) => submid.id === "item-two").subsubmenus || [];
        const navigate = useNavigate();
       return (
    <>
      <div className="my-10 mx-5 grid grid-cols-1 gap-6 justify-center lg:grid-cols-4 md:grid-cols-3">
